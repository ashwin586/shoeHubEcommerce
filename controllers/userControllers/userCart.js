...
exports.userCartGet = async (req, res) => {
  if (req.session.email) {
    try {
      ...
      res.render("user_cart", { loggedIn: true, cart: user.cart, subTotal, errorMessage });
    } catch (err) {
      console.log(err);
    }
  }
...
exports.userCartQuantityUpdate = async (req, res) => {
  ...
  try {
    ...
    if(cartItem.quantity > product.stock ){
      return res.status(400).json({ error: 'Out of stock', productId: product._id});
    }
    ...
    await user.save();
    res.status(200).json({ updatedQuantity: cartItem.quantity, total, fullCartTotal, productId: product._id});
  } catch (err) {
    console.log(err);
  }
};
